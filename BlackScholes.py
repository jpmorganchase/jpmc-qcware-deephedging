...
class BlackScholesProcess:
    ...
    def get_process(self, calculation_date=ql.Date.todaysDate()):
        ...
        volatility = ql.BlackConstantVol(
            0, ql.NullCalendar(), self.sigma, self.day_count)

        # Assume flat term-structure.
        flat_ts = ql.YieldTermStructureHandle(ql.FlatForward(
            0, ql.NullCalendar(), rf_handle, self.day_count))
        dividend_yield = ql.YieldTermStructureHandle(ql.FlatForward(
            0, ql.NullCalendar(), dividend_handle, self.day_count))
    ...
    def gen_path(self, length=None, time_step=None, num_paths=None):
        ...
        rng = ql.GaussianRandomSequenceGenerator(ql.UniformRandomSequenceGenerator(
            time_step, ql.UniformRandomGenerator(self.seed)))
        seq = ql.GaussianMultiPathGenerator(
            self.get_process(), np.linspace(0, length, time_step+1), rng, False)
