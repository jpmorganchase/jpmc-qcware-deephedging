...
from source.qnn import ModuleFn, elementwise, linear, sequential
...
"""
In all the functions below, each layer is of the type ModuleFn i.e. a named tuple of apply_fn and init_fn.
init_fn initialises the network with an initialization params.
apply_fn shows how to do a forward pass using the layers defined in qnn.py which simualtes the network.
"""


def simple_network(
    hps: HyperParams, layer_func: ModuleFn = linear, **kwargs
) -> ModuleFn:
    ...
    def init_fn(key, inputs_shape):
        params = net.init(key, (inputs_shape[0], inputs_shape[1], 2 * inputs_shape[2]))[
            0
        ]
...
def attention_layer(
    ...
    def apply_fn(params, state, key, inputs, **kwargs):
        ...
        case = 0
        dots = jnp.matmul(
            to_w.apply(w_params, None, key, inputs)[0], inputs.transpose(0, 2, 1)
        )
        if case == 0:
...
def attention_network(hps: HyperParams, layer_func=linear, **kwargs) -> ModuleFn:
    ...
    features = hps.n_layers * [
        layer_func(hps.n_features),
        sigmoid,
...
## Layers below are for hardware implementaion
...
def recurrent_network_hardware(
    hps, layer_func: ModuleFn = linear, **kwargs
) -> ModuleFn:
    """Create a Recurrent Network.
    Args:
        n_features: The number of features.
...
def lstm_cell_hardware(hps, layer_func: ModuleFn = linear, **kwargs) -> ModuleFn:
    """Create an LSTM Cell.
    Args:
        n_features: The number of features.
        layer_func: The type of layers to use.
...
def lstm_network_hardware(hps, layer_func: ModuleFn = linear, **kwargs) -> ModuleFn:
    """Create an LSTM Network.
    Args:
        n_features: The number of features.
        layer_func: The type of layers to use.
    """
    preprocessing = [linear(int(hps.n_features / 2)), sigmoid]
    features = [lstm_cell_hardware(hps=hps, layer_func=layer_func)]
    postprocessing = [linear(1), sigmoid]
    layers = preprocessing + features + postprocessing
    net = sequential(*layers)
    return net
