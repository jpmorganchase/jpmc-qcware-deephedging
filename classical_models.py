...
def shared_simple_network(n_features: int = 16, n_layers: int = 3, **kwargs):
    preprocessing = [linear(n_features), sigmoid]
    features = n_layers * [linear(n_features), relu]
...
def simple_network(n_features: int = 16, n_layers: int = 3, **kwargs):
    preprocessing = [linear(n_features), sigmoid]
    features = n_layers * [linear(n_features), relu]
    postprocessing = [linear(1), sigmoid]
    layers = preprocessing + features + postprocessing
...
def lstm_cell(n_features=16):

    _linear = linear(n_features=n_features, with_bias=True)
...
def lstm_network(n_features=16, **kwargs):
    preprocessing = [linear(n_features), sigmoid]
    features = [lstm_cell()]
    postprocessing = [linear(1), sigmoid]
    layers = preprocessing + features + postprocessing
...
def attention_network(n_features: int = 16, n_layers: int = 3, **kwargs):
    n_features = 8
    preprocessing = [linear(n_features), sigmoid]
    features = n_layers * [qnn.linear(n_features), sigmoid,] + [attention_v0(n_features)]
    postprocessing =  [linear(1), sigmoid] 
    layers = preprocessing + features + postprocessing
    net = sequential(*layers)
    return net